...
- **Leaderboard + default benchmark board** (home).
...
### 3.2 Default seed and determinism

- The default benchmark seed is the fixed `YYYY-MM-DD` value `2026-04-21`. This seed was selected after the real-provider experiments in April 2026 because it was already used for the comparable benchmark runs in this repository and is now part of the benchmark contract; it should not be rotated just because calendar time advances.
...
- Default runs MUST use the fixed benchmark seed rather than rolling over with the UTC calendar date. The generator remains available for explicit/custom board previews, tests, and future benchmark modes, but MVP `POST /api/runs` does not accept a user-supplied seed.
...
- **A rule-filtered candidate list** generated by the backend from the visible board state. Target mode lists adjacent unknown cells around current unsunk `X` hits, capped at 12 cells and ordered seed-stably without direction hints. Hunt mode lists up to 12 distributed checkerboard candidates. The cap of 12 keeps prompts compact while giving enough legal alternatives for the model to choose orientation or hunt strategy itself; smaller caps observed in early experiments hid too much of the legal search space and caused brittle repeated misses. Candidate cells adjacent to sunk `S` cells, including diagonals, are removed because ships cannot touch. The cap change affects comparability with earlier experimental runs, so only runs produced under the current contract should be compared as final benchmark results.
...
- `llm_unreachable` - the provider adapter raised `ProviderError { kind: "unreachable" }` (bad key, revoked access, quota exhausted, unsupported model, or caller-side provider rejection). The run terminates without inserting a shot row and without touching schema-error counters.
- `provider_rate_limited` - the provider returned HTTP 429 through the adapter after the retry budget was exhausted. The run terminates without inserting a shot row and without touching schema-error counters because rate limiting is a provider/session failure, not a model schema failure.
...
- `GET /api/board?date=YYYY-MM-DD` -> the rendered PNG for that explicit UTC date, or the fixed default benchmark seed when `date` is absent. Explicit future dates reject; the absent-date default seed is always allowed. Safe to cache per seed.
- `POST /api/runs` -> start a default benchmark run on the fixed seed. Body: `{ providerId, modelId, apiKey, budgetUsd? }`. `budgetUsd` absent, `null`, or `0` means no cap; positive values are stored as floor USD micros; negative or non-numeric values are `invalid_input`. In non-production mock flows, `mockCost` may be supplied for budget smoke tests. Returns `{ runId }`. The API key is consumed synchronously into the task closure and is not echoed.
...
OpenRouter request bodies MUST be gated by model capability metadata captured from `https://openrouter.ai/api/v1/models`: do not send `verbosity`, and send `response_format` only to catalog entries whose `supported_parameters` include `response_format`.
...
- **Typed `ProviderError { kind: "transient" }`** (5xx after retries, TLS, DNS, malformed provider envelope). The run records one `run_shots` row with `result = schema_error`, zero tokens/cost, and redacted `llm_error`, then continues unless the schema-error threshold fires.
- **Typed `ProviderError { code: "rate_limited" }`** (429 after backoff and `Retry-After` handling). The run terminates with `provider_rate_limited` without a shot row and without schema-error debt. A provider/session rate limit is not a model response-format failure.
...
**Why count transient missing-output failures but not provider rejections.** From the benchmark's perspective, a missing model output is a missing shot when the provider accepted the turn and then failed to return usable content, so transient 5xx/network failures and provider timeouts feed the same DNF threshold while remaining distinguishable at the shot-row level. But a 401 on the user's own key, or a 429 imposed by the provider after retries, is not the model's schema discipline. Separating these outcomes keeps the leaderboard honest in both directions.
...
- `/` - leaderboard + default benchmark board. Static HTML with Solid islands hydrated on demand.
