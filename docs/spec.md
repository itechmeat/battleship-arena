...
Every turn, the provider call carries:

- **A PNG image of the current board state.** Cells are rendered as: unknown (empty), hit (filled marker), miss (dot), sunk-ship cells (distinct marker).
- **A short text preamble** listing the ships still afloat by name (for example: "Still afloat: carrier, cruiser, destroyer").
- **A fixed system instruction** describing the output contract in section 3.4.

The board image is the only spatial information. The text preamble contains no coordinates, only names. This matches the brainstorm answer to Q2.

**Why image + ships-remaining, not image + text grid and not image only.** A text grid would give models a second non-visual path to the board state, which undermines the benchmark's claim to exercise spatial reasoning from a visual. The ships-remaining preamble, by contrast, is a derived fact the human opponent already announces in a physical game of Battleship ("you sank my destroyer"), contains no position information, and is cheap enough to include.
...
```json
{ "row": 0, "col": 0, "reasoning": "optional free-form string" }
```

- `row` and `col` are integers in `[0, 9]`.
- `reasoning` is optional, never validated beyond being a string if present, and shown to the viewer as an inline caption.
- Any other shape (wrong keys, out-of-range values, duplicate shots, parse failure, chess-style strings) is a **schema error**.
...
**Why integer `{row, col}` and not chess-style `"A1"`.** Chess-style strings are ambiguous - models regularly disagree with themselves on whether the letter is the row or the column, and on whether `10` is one cell or two. Integer indices are unambiguous, trivially range-checked, and serialized the same by every JSON encoder.
...
3. The task loop (per turn):
   a. Render board image.
   b. Call the provider adapter with image + ships-remaining + system instruction + API key.
   c. Parse and validate the response.
   d. Apply shot (or record schema error).
   e. Emit SSE event on the run's in-memory ring.
   f. Persist the turn row to `run_shots`.
   g. Check terminal conditions.
...
The schema-error DNF threshold is 5 consecutive, not 5 total. A model that self-corrects does not get punished for a single fluke; a model that is genuinely unable to hold a format cannot burn unbounded tokens.
...
**Why count transport failures as schema errors but not 4xx auth failures.** From the benchmark's perspective, a missing model output is a missing shot regardless of cause, so transport faults land on `dnf_schema_errors` honestly. But a 401 on the user's own key is not the model's fault; smearing it onto the model's schema-error row would slander the model. Separating the two outcomes keeps the leaderboard honest in both directions.
...
- Chess-style `"A1"` coordinate format.
