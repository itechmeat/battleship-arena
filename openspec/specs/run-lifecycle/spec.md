...
The backend SHALL expose `runEngine(runId, input, signal, emit, deps)` that drives the full game loop against the provider adapter keyed by `input.providerId` in `deps.providers`. Each turn MUST: (a) compute a `BoardView` from prior shots and the layout from `deps.generate(input.seedDate)`; (b) render the compact text board from that view; (c) compute ships remaining and pass the current consecutive schema-error count to the provider prompt builder; (d) call `adapter.call(callInput, signal)` with `boardText`, `seedDate`, `priorShots`, and the other fields documented by the provider interface, under the per-turn timeout guard; (e) parse `rawText` with `parseShot`; (f) classify the turn as `hit`, `miss`, `sunk`, `schema_error`, `timeout`, or `invalid_coordinate`, insert the appropriate `run_shots` row (raw response truncated to 8 KiB, reasoning to 2 KiB), and emit the corresponding `shot` SSE event including provider usage/timing fields when available; (g) call `reduceOutcome` and exit the loop on a terminal outcome. The engine MUST emit an `open` SSE event before the loop and, except for `aborted_server_restart`, an `outcome` SSE event after the loop and write the finalized `runs` row (`outcome`, `ended_at`, counters). If a terminal provider failure is caused by `ProviderError`, including exhausted 429 retry sequences, the same terminal write MUST also persist the sanitized provider error code, optional HTTP status, and sanitized diagnostic message into the run's terminal-error fields. On `AbortError` whose reason is `server_restart`, the engine MUST exit without writing the terminal row; startup reconciliation writes it instead. The engine MUST read `input.apiKey` only within its own closure; it MUST NOT place it in any variable reachable from outside.

#### Definition: terminal provider-error sanitization

For runEngine terminal provider failures, "sanitized" and "redacted" are equivalent: both refer to the deterministic redaction pass applied before data is persisted to the `runs.terminalErrorCode`, `runs.terminalErrorStatus`, and `runs.terminalErrorMessage` fields. The engine MUST preserve machine-safe values such as `ProviderError.code` and HTTP status when present. The engine MUST sanitize the diagnostic cause before storing `terminalErrorMessage`: strip protocol/source prefixes of the form `<numeric-status> <source>:` such as `403 upstream:`, preserve useful human-readable text such as `Key limit exceeded`, replace API keys, bearer/auth headers, credentials, emails, IP addresses, phone numbers, UUIDs, long alphanumeric identifiers, full URLs, and long stack traces with consistent placeholders such as `[REDACTED]` or `[REDACTED:ID]`, and truncate the resulting UTF-8 text to 2 KiB. The same policy applies whether the provider rejection happens before any `run_shots` rows exist or after previous turns were already written.
...
#### Scenario: Unreachable provider failure persists terminal diagnostics

- **WHEN** `runEngine` receives `ProviderError { kind: "unreachable", code: "quota", status: 403, cause: "403 upstream: Key limit exceeded" }` before a shot row is written for the turn
- **THEN** the run finalizes with `outcome === "llm_unreachable"`, no `run_shots` row is inserted for that failed turn, and the run metadata carries `terminalErrorCode === "quota"`, `terminalErrorStatus === 403`, and a redacted `terminalErrorMessage` containing `Key limit exceeded`

#### Scenario: Exhausted 429 finalizes provider_rate_limited with terminal diagnostics

- **WHEN** `runEngine` receives `ProviderError { kind: "rate_limited", code: "rate_limited", status: 429, cause: "429 upstream: Rate limit exceeded" }`
- **THEN** the run finalizes with `outcome === "provider_rate_limited"`, no shot row is inserted for that failed turn, and the run metadata carries `terminalErrorCode === "rate_limited"`, `terminalErrorStatus === 429`, and a redacted `terminalErrorMessage` containing `Rate limit exceeded`
...
#### Scenario: Rate-limited provider error terminates with clean counters and no shot row

- **WHEN** the provider adapter throws `ProviderError { kind: "rate_limited", code: "rate_limited", cause: "429 upstream: Key limit exceeded", status: 429 }` while `shotsFired`, `consecutiveSchemaErrors`, and `schemaErrors` are all `0`
- **THEN** the engine writes the terminal `runs` row with outcome `provider_rate_limited`, `terminalErrorCode === "rate_limited"`, `terminalErrorStatus === 429`, and `terminalErrorMessage` containing the preserved phrase `Key limit exceeded` after redaction; no `run_shots` row is inserted for the failing turn, and `shotsFired`, `schemaErrors`, and `consecutiveSchemaErrors` all remain at `0`
